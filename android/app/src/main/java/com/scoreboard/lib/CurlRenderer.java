...
public class CurlRenderer implements GLSurfaceView.Renderer {
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		mViewRect.top = -ratio;
		mViewRect.bottom = ratio;
		updatePageRects();

		gl.glMatrixMode(GL10.GL_PROJECTION);
		gl.glLoadIdentity();
		if (USE_PERSPECTIVE_PROJECTION) {
			GLU.gluPerspective(gl, 20f, (float) height/width, .1f, 100f);
		} else {
	...
	private void updatePageRects() {
		if (mViewRect.width() == 0 || mViewRect.height() == 0) {
			...
			mPageRectTop.offset(-mPageRectBottom.width(), 0);

			int bitmapW = (int) ((mPageRectBottom.width() * mViewportWidth) / mViewRect.width());
			int bitmapH = (int) ((mPageRectBottom.height() * mViewportHeight) / mViewRect.height());
			...
			mPageRectTop.set(mPageRectBottom);
			mPageRectTop.right = (mPageRectTop.right + mPageRectTop.left) / 2;
			mPageRectBottom.left = mPageRectTop.right;
